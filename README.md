...
3. 可选：使用页面上的“常见修饰基团”下拉菜单选择修饰，并通过“修饰位置”选择应用到所有匹配残基、N端、C端或指定位置。
4. 也可直接在"修饰"框中输入修饰，格式为逗号分隔的键值对：
...
- 支持页面上的常见修饰基团选择，并可将修饰应用到所有匹配残基、N端、C端或指定位置。
- 位置特异性修饰与序列不匹配时会自动忽略并记录警告。
- 结果显示残基数、残基质量和、失去的水分子数与质量、端修饰总计、以及完整多肽总质量。

氨基酸标准质量表（自由氨基酸分子量）

以下数值为标准自由氨基酸分子量，用于计算多肽质量：
